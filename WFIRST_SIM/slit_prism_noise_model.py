...
# ----------------------------------------------------------------
# Major updates: now have R varying across wavelengths, 
# average value still 50
# incorporating a linespread function this time...
# 
#
# -----------------------------------------------------------------
...
# 2) solar spectrum
# 3) planet-star flux ratio and geometric albedo 
...
# -----------------------------------------------------------------
def stellar_photon_flux_density(wavelength, params):
    # input wavelength unit: microns
    # units of flux returned are photons/m^2/second/micron 
    mstar = params['Mstar'] +5.0*np.log10(params['d_obs']/10.0) # distance modulus to get apparent V band magnitude
    flux_density = (wavelength*10.0**-6.0)/(params['h']*params['c']) \
           * params['fstar_lambda'](wavelength) \
...
def Jp(wl,params):
    return stellar_photon_flux_density(wl, params)*planet_star_fluxratio(wl, params)

def Js(wl,params):
    return stellar_photon_flux_density(wl, params)

def flux_ratio_lsf(wl,width,params,method='rieman'):
    lb = wl*(1.0 - 0.5*wl/params['R'])
    ub = wl*(1.0 + 0.5*wl/params['R'])
    if method=='rieman':
        wlrange = np.arange(lb,ub,0.0001)
        rplist = convolution_p(wlrange,width,params)
        rp = np.trapz(np.array(rplist),x=wlrange)
        rslist = convolution_s(wlrange,width,params)
        rs = np.trapz(np.array(rslist),x=wlrange)
    elif method=='quad':
        rp = quad(convolution_p,lb,ub,args=(width,params))[0]
        rs = quad(convolution_s,lb,ub,args=(width,params))[0] 
    elif method=='astropy':
        width*np.mean(wl)/params['R']
        rp = convolve(Jp(wl,params),Gaussian1DKernel(width),boundary='extend')
        rs = convolve(Js(wl,params),Gaussian1DKernel(width),boundary='extend')

    else:
        print('select method rieman, quad or astropy')
        return -999.0
    return rp/rs
...
def planet_star_fluxratio_selflum(wavelength,params):
    # input wavelength unit: microns     
    return params['selflum'](wavelength)
